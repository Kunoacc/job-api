...
export class CreateOpportunity implements Prisma.OpportunityCreateInput {
  ...
  constructor (data: OpportunityApiResponse) {
    ...
    this.additionalInformation = JSON.stringify(data.details.filter(detail => detail.code !== 'responsibilities' && detail.code !== 'organizations'))
    this.structure = data.details.filter(detail => detail.code === 'team-structure')?.[0]?.content
    this.isRemote = data.place.remote
    ...
    this.members = JSON.stringify(data.members)
          ...
          profile: {
            connectOrCreate: {
              create: {
                ...
                summary: data.details.filter(detail => detail.code === 'organizations')?.[0]?.content || data.details.filter(detail => detail.code === 'reason')?.[0]?.content
              },
              where: {
